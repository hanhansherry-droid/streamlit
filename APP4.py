...
def select_top5_docs(docs, min_words: int = 300, preferred_year: int = 2022):
    ...
    # 2) If not enough, fallback (still dedupe) to ensure 5 outputs
    if len(candidates) < 5:
        fallback = []
        seen = set()
        for d in docs:
    ...
    # 3) Soft preference: pick recent first, then fill (no scoring, no complex ranking)
...
# Dropdown required, but final version must have only ONE model
selected_model = st.sidebar.selectbox(
    "Select LLM",
    ["moonshotai/Kimi-K2-Instruct-0905"]
)

# Markers will use their own key; do NOT include key in PDF
api_key = st.sidebar.text_input("Enter your API key", type="password")

# Keep source screening simple but transparent
...
# Step 2: Select top 5 with simple screening + soft time preference
